...
export const editorialSelectClassName =
    'broadsheet-select text-left disabled:cursor-not-allowed disabled:border-[#E5E7EB] disabled:bg-[#F9FAFB] disabled:opacity-50';
            ...
            <div className="grid gap-6 px-6 py-8 sm:px-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-start">
                <div className="grid gap-4">
                    ...
                    <div className="grid gap-4 lg:grid-cols-[72px_minmax(0,1fr)] lg:items-start">
                        <div className="flex h-[72px] w-[72px] items-center justify-center border border-[#0A0A0A] bg-[#F9FAFB] text-[#0A0A0A]">
                            <Icon className="h-7 w-7" />
                        </div>
                        <div className="grid gap-3">
                            <h2 className="font-heading text-[36px] leading-[1.2] font-bold text-[#0A0A0A]">
                                {title}
                            </h2>
                            <p className="max-w-3xl text-[18px] leading-[1.7] text-[#4B5563]">
                                {description}
                            </p>
                        </div>
                    </div>
...
export const ManagementStatCard = ({
    ...
    return (
        <section
            className={cn(
                'border border-[#E5E7EB] border-t-2 bg-white p-4',
                toneClassMap[tone],
            )}
        >
            <div className="flex items-start justify-between gap-4">
                <div className="grid gap-2">
                    <p className="broadsheet-kicker text-[#4B5563]">{label}</p>
                    <p className="font-heading text-[28px] leading-[1.25] font-bold text-[#0A0A0A]">
                        {value}
                    </p>
                </div>
            ...
            {note ? (
                <p className="mt-3 text-[14px] leading-[1.5] text-[#4B5563]">
                    {note}
                </p>
            ) : null}
        </section>
...
export const ManagementPanel = ({
    ...
    return (
        <section
            className={cn(
                'border border-[#E5E7EB] bg-white p-5 sm:p-6',
                className,
            )}
        >
            {children}
...
export const ManagementPanelHeader = ({
    ...
    return (
        <div className="flex flex-col gap-3 border-b border-[#E5E7EB] pb-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
                <h3 className="font-heading text-[28px] leading-[1.25] font-bold text-[#0A0A0A]">
                    {title}
                </h3>
                {description ? (
                    <p className="mt-2 max-w-2xl text-[16px] leading-[1.7] text-[#4B5563]">
                        {description}
                    </p>
                ) : null}
            </div>
...
export const FilterToolbar = ({
    ...
    return (
        <section
            className={cn(
                'border-y border-y-[#E5E7EB] bg-[#F9FAFB] px-5 py-5',
                className,
            )}
        >
...
export const FilterField = ({
    ...
    return (
        <label className="grid gap-1.5 text-left">
            ...
            {children}
            {hint ? (
                <span className="text-[10px] leading-3 text-[#4B5563]">
                    {hint}
                </span>
            ) : null}
        </label>
